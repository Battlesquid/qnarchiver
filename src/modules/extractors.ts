...
const selectHtml = <T extends HtmlPresence = "Optional">($: cheerio.CheerioAPI, selector: string | cheerio.Element): SelectHtml<T> => {
    const text = unleak(unformat($(selector).text()));
    return (text.trim() === "" ? null : text) as SelectHtml<T>;
};

const selectRawHtml = <T extends HtmlPresence>($: cheerio.CheerioAPI, selector: string | cheerio.Element): SelectHtml<T> => {
